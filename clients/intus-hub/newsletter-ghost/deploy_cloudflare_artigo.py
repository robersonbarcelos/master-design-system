...
TITULO    = "A Cloudflare virou um banco para agentes de IA"
SUBTITULO = "O produto nao e para voce. E para o agente que voce vai construir."

IMG_DIR = Path("SKILLS/Master-social-design-system/clients/intus-hub/newsletter-ghost/imagens-publicacao/10-08")
...
def upload(i: int, data: bytes) -> str:
    token = ghost_token()
    files = {
        "file": (f"cloudflare-artigo-img{i}.jpg", data, "image/jpeg"),
        "purpose": (None, "image"),
    }
    r = requests.post(
...
# ── Main ─────────────────────────────────────────────────────────
print("\n=== DEPLOY ARTIGO — A CLOUDFLARE VIROU UM BANCO PARA AGENTES DE IA ===\n")
...
payload = {
    "posts": [{
        ...
        "lexical": lexical,
        "status": "draft",
        "tags": [{"name": "IA"}, {"name": "Agentes"}, {"name": "Cloudflare"}],
    }]
}
r = requests.post(
